...
# --------------------------------------
    ...
    if w_name!='Select':
        def state_list():
            st.session_state['states'] = my_states
        ...
        with st.form("edit_form", clear_on_submit=True):
            with left:
                with st.expander("General Information"):
                    worker_name = st.text_input("Worker Name",value=selected_worker['worker_name'].values[0],key=1,label_visibility="hidden")
            ...
            with right:
                with st.expander("Work Related Info"):
                    ...
                    worker_state = st.selectbox("State",options=selected_worker['worker_state'].values[0],on_change= state_list,key=8)
