...
if selected == "Edit Worker Information":
    ...
    if w_name!='Select':
        ...
        with st.form("edit_form", clear_on_submit=True):
            with left:
                with st.expander("General Information"):
                    worker_name = st.text_input("Worker Name",selected_worker['worker_name'].values[0])
            ...
            with right:
                with st.expander("Work Related Info"):
                    worker_current_company = st.text_input("Current Working Company Name",selected_worker['worker_current_company'].values[0])
                    worker_current_workplace = st.text_area("Address",selected_worker['worker_current_workplace'].values[0])
                    worker_state_new = st.selectbox("State",kwargs = {"States":my_states})
                    worker_pic = st.text_area("Current Company PIC(Person-in-Charge) Details",selected_worker['worker_pic'].values[0])
            ...
            with st.expander("Other"):
                comments = st.text_input("Comments(if any)",selected_worker['comments'].values[0])
